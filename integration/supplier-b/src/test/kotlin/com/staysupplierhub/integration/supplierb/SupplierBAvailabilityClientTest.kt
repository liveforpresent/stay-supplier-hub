...
class SupplierBAvailabilityClientTest : FunSpec({
    test("availability request includes the Supplier B API key") {
        AvailabilityRequestStub().use { stub ->
            fetch(stub.baseUrl, targets("property-a"), condition()) shouldBe "{}"
    ...
    test("availability request preserves stay dates and guest composition") {
        AvailabilityRequestStub().use { stub ->
            fetch(stub.baseUrl, targets("property-a"), condition())

            stub.query shouldBe mapOf(
                "propertyIds" to "property-a",
    ...
    test("availability request joins external property codes with commas") {
        AvailabilityRequestStub().use { stub ->
            fetch(stub.baseUrl, targets("property-a", "property-b"), condition())

            stub.query.getValue("propertyIds") shouldBe "property-a,property-b"
        }
    }
    ...
    test("a single availability request rejects more than fifty property codes") {
        AvailabilityRequestStub().use { stub ->
            shouldThrow<IllegalArgumentException> {
                fetch(stub.baseUrl, targets(*(1..51).map { "property-$it" }.toTypedArray()), condition())
            }
...
private fun client(baseUrl: String) = SupplierBAvailabilityClient(
