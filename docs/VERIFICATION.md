...
| `V-WIRE-SUP-04` | P1 | APPLICATION_UNIT / review | Construct `SupplierId("C")` without changing shared identity code | Valid identifier; support still absent until adapter/config registration | PASSING |
| `V-WIRE-SUP-05` | P1 | APPLICATION_UNIT | Blank SupplierId | Rejected by value-object invariant | PASSING |
| `V-WIRE-SUP-06` | P1 | CROSS_MODULE / CONFIG_BINDING | Supplier A and B use different runtime values | Each Supplier client/adapter receives only its own configuration | PASSING |
...
`V-WIRE-SUP-04..05` evidence: `:catalog:api:test --tests com.staysupplierhub.catalog.api.CatalogIdsTest`.
