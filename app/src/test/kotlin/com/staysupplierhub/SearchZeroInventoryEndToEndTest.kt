...
@Testcontainers
class SearchZeroInventoryEndToEndTest @Autowired constructor(
    ...
    @Test
    fun `a valid zero-inventory stay date retains the offer with zero available rooms`() {
        ...
        val supplierAOffer = body["offers"].single { it["supplier"].asText() == "A" }
        assertEquals(0, supplierAOffer["availableRooms"].asInt())
        assertTrue(supplierAOffer["propertyId"].isTextual)
        assertTrue(supplierAOffer["roomTypeId"].isTextual)
        assertEquals(3, body["offers"].single { it["supplier"].asText() == "B" }["availableRooms"].asInt())
    }
    ...
    companion object {
        ...
        private fun mockSupplierBaseUrl(): String =
            requireNotNull(System.getProperty("e2e.mock-supplier.base-url")) {
                "e2e.mock-supplier.base-url must be configured by the e2eZeroInventoryTest task"
            }
    }
}
