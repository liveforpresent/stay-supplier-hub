...
@Testcontainers
class SearchPartialSupplierFailureEndToEndTest @Autowired constructor(
    ...
    @Test
    fun `supplier A HTTP failure preserves supplier B offer as partial search`() {
        ...
        assertEquals(listOf("A"), body["degradedSuppliers"].map { it.asText() })
        assertEquals(1, body["offers"].size())
        assertEquals("B", body["offers"][0]["supplier"]?.asText(), response.body())
    ...
    companion object {
        ...
        private fun mockSupplierBaseUrl(): String =
            requireNotNull(System.getProperty("e2e.mock-supplier.base-url")) {
                "e2e.mock-supplier.base-url must be configured by the e2ePartialSupplierFailureTest task"
            }
    }
}
