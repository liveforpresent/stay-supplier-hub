...
@Testcontainers
class SearchCatalogBaselineFailureEndToEndTest @Autowired constructor(
    ...
    @Test
    fun `fresh supplier A catalog failure closes search despite supplier B bootstrap success`() {
        assertFalse(catalogReadiness.isReady())
        assertEquals(setOf(SupplierId("A")), catalogReadiness.unavailableSupplierIds())
        assertEquals(0, catalogPersistenceAdapter.findAllBySupplier(SupplierId("A")).size)
        assertEquals(1, catalogPersistenceAdapter.findAllBySupplier(SupplierId("B")).size)
        ...
        assertEquals(setOf("A"), body["suppliers"].map { it.asText() }.toSet())
    }
    ...
    companion object {
        ...
        private fun mockSupplierBaseUrl(): String =
            requireNotNull(System.getProperty("e2e.mock-supplier.base-url")) {
                "e2e.mock-supplier.base-url must be configured by the e2eCatalogBaselineFailureTest task"
            }
    }
}
